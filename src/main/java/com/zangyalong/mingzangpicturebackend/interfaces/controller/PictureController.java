...
import com.zangyalong.mingzangpicturebackend.infrastructure.api.aliyunai.AliYunAiApi;
import com.zangyalong.mingzangpicturebackend.infrastructure.api.aliyunai.CreateOutPaintingTaskResponse;
import com.zangyalong.mingzangpicturebackend.infrastructure.api.aliyunai.GetOutPaintingTaskResponse;
import com.zangyalong.mingzangpicturebackend.infrastructure.api.imagesearch.ImageSearchApiFacade;
import com.zangyalong.mingzangpicturebackend.infrastructure.api.imagesearch.model.ImageSearchResult;
...
import com.zangyalong.mingzangpicturebackend.interfaces.assembler.PictureAssembler;
...
import com.zangyalong.mingzangpicturebackend.shared.auth.annotation.SaSpaceCheckPermission;
...
import com.zangyalong.mingzangpicturebackend.interfaces.vo.picture.PictureTagCategory;
...
import com.zangyalong.mingzangpicturebackend.domain.picture.valueobject.PictureReviewStatusEnum;
...
//            }
    ...
    public BaseResponse<Boolean> editPicture(@RequestBody PictureEditRequest pictureEditRequest, HttpServletRequest request) {
        ...
        Picture picture2 = PictureAssembler.toPictureEntity(pictureEditRequest);

        // 构造 UpdateWrapper
        UpdateWrapper<Picture> updateWrapper = new UpdateWrapper<>();
        updateWrapper.eq("id", oldPicture.getId()); // 指定主键条件，批量更新则使用 in 传递多条
        if(spaceId == null || spaceId == 0) {
            updateWrapper.eq("spaceId", 0);
        }else {
            updateWrapper.eq("spaceId", spaceId);
        }
        // 补充条件 spaceId=xxx
        // 操作数据库
        boolean result = pictureApplicationService.update(picture2, updateWrapper);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
